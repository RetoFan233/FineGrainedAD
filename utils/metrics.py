...
def metric_cal_img(img_scores, gt_list, map_scores=None):
    ...
    img_scores_ = (img_scores + max_map_scores) / 2
    ...
    img_roc_auc = max(roc_auc_score(gt_list, img_scores_), img_roc_auc)

    result_dict = {'i_roc': img_roc_auc * 100}

    return result_dict

def metric_cal_img_multiple_metrics(img_scores, gt_list, map_scores=None):
    # calculate image-level ROC AUC score
    max_map_scores = map_scores.reshape(map_scores.shape[0], -1).max(axis=1)
    ...
    # pro_auc_score = cal_pro_metric(gt_list, img_scores, fpr_thresh=0.3)
    
    # max_f1_score = calculate_max_f1_region(gt_list, img_scores, pro_thresh=0.6)

    result_dict = {'i_roc': img_roc_auc * 100, 'i_aupr': calculate_aupr(gt_list, img_scores) * 100}
    # result_dict = {'i_roc': img_roc_auc * 100, 'img_max_f1': max_f1_score * 100}
    # result_dict = {'i_roc': img_roc_auc * 100, 'img_pro_auc': pro_auc_score * 100, 'img_max_f1': max_f1_score * 100}
...
def metric_cal_pix_multiple_metrics(map_scores, gt_mask_list):
    ...
    # max_f1_score = calculate_max_f1_region(gt_mask_list, map_scores, pro_thresh=0.6)

    result_dict = {'p_roc': per_pixel_rocauc * 100, 'p_pro': pro_auc_score * 100}
    # result_dict = {'p_roc': per_pixel_rocauc * 100, 'pix_pro_auc': pro_auc_score * 100, 'pix_max_f1': max_f1_score * 100}
