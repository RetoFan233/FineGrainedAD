...
def write_results_multiple(results:dict, cur_class, total_classes, csv_path):
    keys_ = list(results.keys())
    keys = ['i_roc', 'p_roc', 'i_aupr', 'p_pro']
...
def save_metric_mutiple(metrics, total_classes, class_name, dataset, csv_path):
    # if dataset != 'mvtec':
    for indx in range(len(total_classes)):
        total_classes[indx] = f"{dataset}-{total_classes[indx]}"
    class_name = f"{dataset}-{class_name}"
    write_results_multiple(metrics, class_name, total_classes, csv_path)
