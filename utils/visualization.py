...
def plot_sample_compare(names, imgs, scores_: dict, gts, save_folder=None):
    # get subplot number
    total_number = len(imgs)
    ...
    for idx in range(total_number):
        ...
        for key in scores:
            ...
            ss = scores[key][idx].copy()
            #计算ss的最大值
            ss_max = np.max(ss)
            ss[ss > 0.8 * ss_max] = 255
            ss[ss <= 0.8 * ss_max] = 0
            ss = cv2.applyColorMap(ss, cv2.COLORMAP_JET)
            visz_map = cv2.addWeighted(ss, 0.5, imgs[idx], 0.5, 0)
            cv2.imwrite(os.path.join(save_folder, f'{names[idx]}_{key}_binary.jpg'),
                        visz_map)
...
def tsne_feature(features, n_components=2, method='TSNE', perplexity=30):
    assert method in valid_feature_visualization_methods
    assert n_components in [2, 3]
