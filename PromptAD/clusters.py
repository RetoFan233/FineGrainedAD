...
def index_points(points, idx):
    ...
    Returns:
        new_points:, indexed points data, [B, S, C] S is the number of cluster

    Args:
        points: input points data, [B, N, C]
        idx: sample index data, [B, S] #S为聚类中心的数量
    """
...
def cluster_dpc_knn(token_dict, cluster_num, k=5, token_mask=None):
    ...
    with torch.no_grad():
        ...
        mask = density[:, None, :] > density[:, :, None] #mask为1的地方表示density[i] > density[j]
        mask = mask.type(x.dtype)
        dist_max = dist_matrix.flatten(1).max(dim=-1)[0][:, None, None]
        dist, index_parent = (dist_matrix * mask + dist_max * (1 - mask)).min(dim=-1) #mask为1的地方取dist_matrix的值，mask为0的地方取dist_max的值

        # select clustering center according to score
        score = dist * density
        # print("score shape:", score.shape)
        
        # print("score:", score) #torch.Size([1, 225]) B=1, N=225
        _, index_down = torch.topk(score, k=cluster_num, dim=-1) #此时选出每个样本的聚类中心的索引 #这里可以将index_down替换为语义信息指导得到的聚类中心
        # print("index_down shape:", index_down.shape) #torch.Size([1, 2]) B=1, cluster_num=2
        # print("index_down:", index_down)
        # assign tokens to the nearest center
        # print("dist matrix shape:", dist_matrix.shape)
        dist_matrix = index_points(dist_matrix, index_down) #[1,225,225] - > [1, 2, 225]
        # print("dist_matrix after index_points shape:", dist_matrix.shape)
        # print("dist_matrix after index_points:", dist_matrix)
        
        idx_cluster = dist_matrix.argmin(dim=1)
        # print("idx_cluster after argmin:", idx_cluster)

        # make sure cluster center merge to itself
        idx_batch = torch.arange(B, device=x.device)[:, None].expand(B, cluster_num)
        idx_tmp = torch.arange(cluster_num, device=x.device)[None, :].expand(B, cluster_num)
        idx_cluster[idx_batch.reshape(-1), index_down.reshape(-1)] = idx_tmp.reshape(-1)
        # print("idx_cluster after merge to itself:", idx_cluster)

    return idx_cluster, cluster_num


def language_guided_cluster_dpc_knn(token_dict, cluster_num, k=5, token_mask=None, semantic_info=None):
    """Cluster tokens with DPC-KNN algorithm.
    Return:
        idx_cluster (Tensor[B, N]): cluster index of each token.
        cluster_num (int): actual cluster number. The same with
...
def merge_tokens(token_dict, idx_cluster, cluster_num, token_weight=None):
    ...
    # print("idx_token_new:", idx_token_new)
...
class CTM(nn.Module):
    ...
    def forward(self, token_dict, sample_ratio=None):
        ...
        if token_dict["mask"] is not None:
            token_weight.masked_fill_((1 - token_dict["mask"]).to(torch.bool), float("-inf")) #token_dict为0的位置填充为负无穷
        ...
        idx_cluster, cluster_num = language_guided_cluster_dpc_knn(
            token_dict, cluster_num, k, token_mask=token_dict["mask"], semantic_info=token_dict["semantic_info"])
