...
#     """
#     Use QK Normalization(modified from HunyuanDiT).
#     """
#     def __init__(self,
#                  qdim,
#                  kdim,
...
#         # if dtpye == 'fp16':
#         #     self.q_norm = norm_layer(self.head_dim, elementwise_affine=True, eps=1 / 65530) if qk_norm else nn.Identity()
#         #     self.k_norm = norm_layer(self.head_dim, elementwise_affine=True, eps=1 / 65530) if qk_norm else nn.Identity()
#         # else:
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4) # shape:[3, B, num_heads, N, C//num_heads]
...
class QCrossAttention(nn.Module):
    def __init__(self, out_dim, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        self.kv_proj = nn.Linear(dim, dim * 2, bias=qkv_bias, dtype=dtype)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(out_dim, dim, dtype=dtype)
        self.proj_drop = nn.Dropout(proj_drop)
        self.settings = settings
    ...
    def forward(self, x, y):
        ...
        q = self.q_proj(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3) # shape:[B, num_heads, N, C//num_heads]
        kv = self.kv_proj(y).reshape(B, N, 2, self.num_heads, C // self.num_heads).permute(0, 2, 3, 1, 4) # shape:[B, 2, num_heads, N, C//num_heads]
        k, v = kv[:, 0], kv[:, 1]
...
#Query Former(consists of two parallel CrossAttention)
#which aims to use learnable queries to capture the global context information from two different tensor inputs.
class QueryFormer(nn.Module):
    def __init__(self, num_query_token, embed_dim, num_heads=8, qkv_bias=True, precision='fp16'):
        super().__init__()
        ...
        self.cross_attn1 = QCrossAttention(embed_dim, embed_dim, num_heads, qkv_bias=True, dtype=dtype)
        self.cross_attn2 = QCrossAttention(embed_dim, embed_dim, num_heads, qkv_bias=True, dtype=dtype)
        
        #learnable quries()
        self.query_tokens = nn.Parameter(
            torch.zeros(num_query_token, embed_dim, dtype=dtype)
        )
        self.query_tokens.data.normal_(mean=0.0, std=0.02)
        
        self.fuse_layer = nn.Linear(embed_dim * 2, embed_dim, dtype=dtype)

    def forward(self, y1, y2):
        # print("Y1 Shape, Y2 Shape:",y1.shape, y2.shape)
        # print("Query_tokens Shape:", self.query_tokens.shape)
        out1 = self.cross_attn1(self.query_tokens.unsqueeze(1), y1.unsqueeze(1)).squeeze(1)
        out2 = self.cross_attn2(self.query_tokens.unsqueeze(1), y2.unsqueeze(1)).squeeze(1)
        
        #fuse the two outputs to get the final output
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, save_feature = False):
...
class V2VTransformer(nn.Module):
    ...
    def __init__(
        ...
        # class embeddings and positional embeddings
        scale = width ** -0.5 # x * x = 768 -> x = 
        self.class_embedding = nn.Parameter(scale * torch.randn(width)) # cls token
        self.positional_embedding = nn.Parameter(scale * torch.randn(self.grid_size[0] * self.grid_size[1] + 1, width)) # positional embedding

        # setting a patch_dropout of 0. would mean it is disabled and this function would be the identity fn
        self.patch_dropout = PatchDropout(patch_dropout) if patch_dropout > 0. else nn.Identity() # no patch dropout
        ...
        # old
        self.transformer.resblocks[2].register_forward_hook(hook_t1) #layer3 token feature w/o cls token
        self.transformer.resblocks[7].register_forward_hook(hook_t2) #layer8 token feature w/o cls token
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.attn == None: #this path

            if self.layers == 12:
                #ViT-B
                print("surgery 12")
                for i in range(1, 13):  # surgery 7, maskclip 2 (modified by VV-Attention)
        ...
        # class embeddings and positional embeddings
        # print("before class_embedding:", x.shape)
        
        x = torch.cat(
            [self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device),
             x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        # print("after class_embedding:", x.shape)
        ...
        x, x_ori = x # x from VV-Attention x_ori from original CLIP
        x[0] = x_ori[0] # modify cls token
...
class V2VPatchTransformer(nn.Module):
    output_tokens: torch.jit.Final[bool]
    ...
    def __init__(
        ...
        self.large_grid_size = (self.grid_size[0] - 2, self.grid_size[1] - 2)
        self.mid_grid_size = (self.grid_size[0] - 1, self.grid_size[1] - 1)
        ...
        self.init_parameters()
        self.attn = None
        print("Now in V2VPatchTransformer")
    ...
    def forward(self, x: torch.Tensor, mask = None, proj = False, save_feature = False):
        ...
        for mask_scale in mask:
            ...
            x_scale, x_ori = x_scale # x from VV-Attention x_ori from original CLIP
            x_scale[0] = x_ori[0]
...
class VisionPatchTransformer(nn.Module):
    output_tokens: torch.jit.Final[bool]
    ...
    def __init__(
        ...
        print("Now in VisionPatchTransformer")
