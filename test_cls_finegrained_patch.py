...
def test(model,
    ...
    result_dict = metric_cal_img_multiple_metrics(np.array(scores_img), gt_list, np.array(score_maps))
...
def main(args):
    ...
    img_dir, csv_path, check_path = get_dir_from_args('SEG', **kwargs)
    ...
    i_aupr = round(metrics['i_aupr'], 2)
    object = kwargs['class_name']
    print(f'Object:{object} =========================== Image-AUROC:{i_roc} Image-AUPR:{i_aupr}\n')
...
def get_args():
    ...
    parser.add_argument("--root-dir", type=str, default="./result_finegrained_0905_w03_fp32")
